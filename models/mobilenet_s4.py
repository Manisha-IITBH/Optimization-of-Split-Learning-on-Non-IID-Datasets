...
num_unfrozen_center_layers = 6          #exp with 1 or 2(max)
...
class center(nn.Module):
    def __init__(self, pretrained=False):
        ...
        model.classifier[0] = nn.Linear(576,512)
        block1 = model_children[0][1:13]
        block2 = model_children[1:2]+[nn.Flatten()]
        block3 = model_children[2][:3]
        #model_children = model_children[0][1:13]nn.Sequential(model_children[1])
        #center_model_length = len(model_children)
        self.center_model = nn.Sequential(*block1,*block2,block3)
        center_model_length = len(list(self.center_model))
