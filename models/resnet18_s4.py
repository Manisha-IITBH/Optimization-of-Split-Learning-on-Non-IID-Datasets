...
class center(nn.Module):
    def __init__(self, pretrained=False):
        ...
        front_layers = model_children[num_front_layers:7]
...
class back(nn.Module):
    def __init__(self, pretrained=False, output_dim=10):
        ...
        # Get the layers up to the 8th layer
        front_layers = model_children[:7]

        # Split the 8th layer into two BasicBlocks
        #block1 = front_layers[-1][0]
        block2 = model_children[7]
        block3 = model_children[8]
        ...
        self.back_model = nn.Sequential(block1, block2, block3,*back_layers)
        num_back_layers = len(list(self.back_model.children()))
        print(num_back_layers)
        if pretrained:
